...
def main(argv):
    ...
    for train_id in tqdm(train_ids):
        ...
        y_pred_path = os.path.join(FLAGS.prediction, train_id)
        y_true = np.array(Image.open(y_true_path)).astype(float)
        y_pred = np.array(Image.open(y_pred_path)).astype(float)
        y_true = np.round(y_true / 65535.).astype(int)
        y_pred = np.round(y_pred / 255.).astype(int)
